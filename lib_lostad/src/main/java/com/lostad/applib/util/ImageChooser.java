...
public class ImageChooser {
	...
	public static final int REQUEST_CODE_CROP     = 65535;//裁剪
	public static final int REQUEST_CODE_PIC      = 65534;//图片
	public static final int REQUEST_CODE_PHOTO    = 65533;//相机
	...
	public static void showImagePickerForResult(final Activity context){
			...
			public void onClick(DialogInterface dialog, int which) {
				switch (which) {
					case 0:
						...
						context.startActivityForResult(openCameraIntent, REQUEST_CODE_PHOTO);
						break;
					case 1:
						Intent intent = new Intent();
						intent.setType("image/*");
						intent.setAction(Intent.ACTION_PICK);
						context.startActivityForResult(intent, REQUEST_CODE_PIC);
						break;
	...
	public static void onActivityResult(Activity ctx,int requestCode,boolean isCrop, Intent data,ImageCallback callback) {

		switch (requestCode) {//上次选择的索引
			case REQUEST_CODE_PHOTO://相机
				if(isCrop){//裁剪
					Uri uri = null;
					if (data != null) {
						uri = data.getData();
			...
			case REQUEST_CODE_PIC: //选择的照片的数据
	...
	//截取图片
	private static void cropImage(Activity ctx,Uri uri, int outputX, int outputY, int requestCode){
